...
def attack_wav(dataloader, x, y, model, n_iter, epsilon, alpha, rand_init_uniform=True, rand_init_normal=False,
               clip_min_max=True):
    model.eval()
    correct = 0
    total = 0
    ...
    for i in range(n_iter):
        print(audio.shape)
        padded_audio = pad_audio(audio + delta, size=16000)
...
def attack(dataloader, example_idx, model, n_iter=10, eps=0.01, alpha=0.01 / 2, rand_init=True):
    ...
    adv_wav = attack_wav(dataloader, x, y, model, n_iter, eps, alpha, rand_init)
    return adv_wav
